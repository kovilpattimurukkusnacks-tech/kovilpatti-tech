...
> **Kovilpatti Snacks note**: `front-end/tsconfig.app.json` does not set `strict: true` explicitly (TS defaults it off). Flag this as a 🟡 Important finding the first time it's noticed in a review, but don't re-litigate it in every review — it's a known gap, not a new regression, unless the diff under review changes `tsconfig.json` itself.

### 7. Framework-Specific Considerations

**React + TypeScript** (this is what `front-end/` uses — React 19, MUI v9, TanStack Query 5, React Router 7):
...
- MUI v9 specifics: the `containedPrimary` styleOverrides slot is gone — flag any attempt to use it (see repo-root `CLAUDE.md`); brand gold gradient should come from `theme.ts`'s `GOLD_GRADIENT` export, not a hardcoded gradient string, when touching primary-action surfaces.

**Node.js + TypeScript**:
- Not applicable to this repo's backend — the API is **.NET 9 / C#** (`Backend/`), not Node. Skip this subsection when reviewing `Backend/`.

**Testing**:
- This repo has **no test runner configured** (no Jest/Vitest in `front-end/package.json`, no `*.test.ts(x)` files). Don't flag "missing test coverage" as a per-PR finding — there is no existing test suite to be consistent with. If the user is introducing tests for the first time, that's a separate, larger discussion (test framework choice) rather than a review comment.

### 8. Automated Checks to Recommend

Suggest running these tools if not already in use:
- **TypeScript compiler**: `tsc -b` (this repo's actual `build` script) or `tsc --noEmit` for a check-only run
- **ESLint**: `npm run lint` (already configured — `js.configs.recommended` + `tseslint.configs.recommended` + React hooks/refresh plugins; note this is the **non-type-checked** `recommended` preset, not `recommended-type-checked`, so ESLint alone won't catch everything `tsc` would)
- **Prettier**: not currently configured in this repo — don't assume it's present
...
7. **Testing and docs**: Coverage, clarity, maintainability (see §7 testing note — don't flag missing tests as a gap in this repo)
...
2. Check for any `tsconfig.json` in the project (this repo: `front-end/tsconfig.json` references `tsconfig.app.json` + `tsconfig.node.json`)
...
Scope note for this repo: only applies to `front-end/**/*.ts(x)`. The backend (`Backend/`) is C#/.NET and out of scope for this skill.
